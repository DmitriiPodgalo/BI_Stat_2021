...
read_data <- function(path = "../data/", pattern = '*.csv', ...) {
  do.call(rbind, lapply(list.files(path, pattern), function(file) read.csv(paste0(path, file))))
}
td <- read_data()
```
...
### # 14
```{r}
stat_tables <- function(varr) {
v <- td %>% 
  distinct_at(vars(Name), .keep_all = T) %>% 
  filter(Sex == c('M', 'F')) %>% 
  t.test(.[,varr] ~ .[, 'Sex'], data = ., na.action = na.omit)

data.frame(t.value = v$statistic, df = v$parameter, p.value = v$p.value, 'F mean' = v$estimate[1], 'M mean' = v$estimate[2], row.names = c()) %>%
  kbl(caption = paste(varr, 'comparison')) %>%
...
3. Below we can see that US has more "NA" medals, it means that sportsmen count can correlate with medals count and US just has more sportsmen.
